...
const Request: React.FC = () => {
  ...
  const { loading: couriersLoading } = useGetCourier();

  useEffect(() => {
    if (currentUser?.id) {
      dispatch(getBookings({ userId: currentUser.id }));
    }
  }, [currentUser?.id, dispatch]);

  if (loading || couriersLoading) return <div>Loading...</div>;
  if (error) return <div>Error: {error}</div>;

  const getCourierName = (courierId?: string) => {
    const courier = couriers.find(c => c.id === courierId);
    return courier ? `${courier.firstName} ${courier.lastName}` : 'Unknown Courier';
  };
            ...
            <li key={booking.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-sm font-medium text-indigo-600 truncate">{getCourierName(booking.courierId)}</div>
