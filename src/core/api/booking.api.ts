import { collection, getDocs, addDoc, deleteDoc, doc, query, where, Query, type DocumentData, } from "firebase/firestore";
...
export const getBookingsAPI = async (filter: { userId?: string; courierId?: string }): Promise<IBooking[]> => {
  let q: Query<DocumentData> = bookingsCollection;

  if (filter.userId) q = query(bookingsCollection, where("userId", "==", filter.userId));
  if (filter.courierId) q = query(bookingsCollection, where("courierId", "==", filter.courierId));
